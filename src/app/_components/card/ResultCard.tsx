...
const ResultItem: React.FC<{ label: string; value: string | number; color: string; isScore?: boolean }> = ({ label, value, color, isScore }) => {
    return (
        <div className="flex flex-col md:flex-row justify-center items-center text-3xl gap-1 mb-6 px-8 text-center">
...
const ResultCard: React.FC<ResultCardProps> = ({ correctAnswers, wrongAnswers, score, todayStreakQuizLeft, isChallenge, rank, textColor, badgeColor }) => {
    ...
    return (
        <div className="bg-white p-2 md:p-6 rounded-xl shadow-2xl border-2 border-gray-200 w-full max-w-4xl text-center">
            {todayStreakQuizLeft != null && ( //스트릭 결과라면 해당 값을 프롭으로 전달. 이 값이 존재하면 해당 문구 표시
                <h1 className="md:text-5xl text-2xl font-extrabold text-lime-600 mb-12 mt-4">
            ...
            {isChallenge && (
                <>
                    ...
                    <p className="md:text-3xl text-xl font-extrabold text-gray-700 mb-8">채점 결과</p>
                </>
            )}
